...
describe 'powershell provider:', if: (os[:family] == 'windows') do
  ...
  describe 'should not leak environment variables across calls to single session' do
    ...
    after(:each) do
      run_shell(PuppetLitmus::Utils.interpolate_powershell("Remove-Item Env:\\superspecial -ErrorAction Ignore;exit 0"))
      run_shell(PuppetLitmus::Utils.interpolate_powershell("Remove-Item Env:\\outside -ErrorAction Ignore;exit 0"))
    end
    ...
    it 'should see environment variables set outside of session' do
      ...
      run_shell(PuppetLitmus::Utils.interpolate_powershell("$env:outside='1'"))
